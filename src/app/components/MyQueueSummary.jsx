      ...
      <ul className="space-y-2 mb-3">
        {tickets.map(t => (
          <li key={t.id} className="flex items-center justify-between">
            ...
            <button onClick={() => onRemove(t.id)} className="text-xs text-red-600">Remove</button>
          </li>
        ))}
      </ul>
      <button onClick={onClear} className="px-3 py-1 rounded bg-red-600 text-white">Clear Queue</button>
    </div>
  );
}